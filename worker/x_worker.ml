...
let run () =
  Brr.Console.log [ "Worker loaded - VERSION 2025-01-28-12:30 with X_canvas support" ];
    ...
    (try
     ...
     with _ ->
       ...
       (try
        ...
        with err ->
          Brr.Console.error [ "Error processing widget message:"; Printexc.to_string err ]))
  in
  let _listener = Brr.Ev.listen Brr_io.Message.Ev.message handle_message Brr.G.target in
  ()

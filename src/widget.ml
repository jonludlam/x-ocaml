...
let setup_event_listeners canvas_el widget_id cell_id worker =
  let send_event event = Client.post worker (Widget_event (cell_id, widget_id, event)) in
...
let create ~cell_id ~widget_id worker = function
  | X_protocol.Canvas { width; height } ->
      ...
      let canvas_el = El.canvas ~at:[At.int (Jstr.v "width") width; At.int (Jstr.v "height") height] [] in
      ...
      let msg =
        Jv.obj
          [| ("type", Jv.of_string "offscreen_canvas"); ("widget_id", Jv.of_int widget_id); ("canvas", Offscreen_canvas.to_jv offscreen) |]
      in
      ...
      { id = widget_id; cell_id; kind = Canvas { canvas = canvas_el; offscreen }; container }
